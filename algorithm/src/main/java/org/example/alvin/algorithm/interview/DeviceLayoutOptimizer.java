...
public class DeviceLayoutOptimizer {
  ...
  /** 主入口方法 */
  public LayoutResult findOptimalLayout(
      List<String> devices,
   ...
   */
  private void generatePlacementsRecursive(
      int deviceIdx, int[] currentPlacement, boolean[] locationUsed) {
    // --- 递归基线条件：所有设备都已安放 ---
    // 这是"有哪些可能性"中的一种
    if (deviceIdx == m) {
  ...
  public static void main(String[] args) {
    ...
    // 定义设备连接关系
    List<DeviceConnection> deviceConnections =
        Arrays.asList(new DeviceConnection("A", "B"), new DeviceConnection("A", "C"));
    ...
    LayoutResult result =
        optimizer.findOptimalLayout(devices, locations, deviceConnections, locationConnections);
    ...
    List<LocationConnection> locationConnections2 =
        Arrays.asList(
            new LocationConnection("P1", "P2", 1), new LocationConnection("P1", "P3", 3)
            // P2 和 P3 不直连，但最短路径是 P2-P1-P3，距离为 1+3=4
            );

    DeviceLayoutOptimizer optimizer2 = new DeviceLayoutOptimizer();
    LayoutResult result2 =
        optimizer2.findOptimalLayout(devices, locations, deviceConnections, locationConnections2);
