package org.example.alvin.algorithm.leetcode.deepfirstsearch;
...
public class LCP365 {
  ...
  private static boolean canMeasureWater(int x, int y, int z) {
    ...
    while (!queue.isEmpty()) {
      ...
      addIntoQueue(
          queue, visited, new AbstractMap.SimpleEntry<>(remainX - moveSize, remainY + moveSize));
  ...
  private static void addIntoQueue(
