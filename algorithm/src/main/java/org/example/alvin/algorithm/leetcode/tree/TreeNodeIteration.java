...
public class TreeNodeIteration {
  ...
  static class BinaryTreeTraversalIterative {
     ...
     * 迭代思路：
     * 1. 使用一个栈，将根节点压入。
     * 2. 当栈不为空时，弹出一个节点 (curr)。
     * 3. 访问该节点 (curr.val)。
     * 4. [关键] 先将其 右子节点 压入栈 (如果存在)。
     * 5. [关键] 再将其 左子节点 压入栈 (如果存在)。
     * (因为栈是后进先出 LIFO，所以先压右再压左，才能保证左先于右被访问)
     */
    List<Integer> preorderTraversal(TreeNode root) {
     ...
     * 1. 使用一个栈和一个指针 (curr)。
     * 2. 指针 curr 指向 root。
     * 3. [循环] 当 curr 不为 null 或 栈不为空时：
     * a. [一路向左] 当 curr 不为 null 时，将 curr 压入栈，并令 curr = curr.left。
     * b. [访问] 当 curr 为 null 时 (说明左边到底了)，从栈中弹出一个节点 (node)。
     * c. 访问该节点 (node.val)。
     * d. [转向右] 令 curr = node.right (准备处理右子树)。
     */
    List<Integer> inorderTraversal(TreeNode root) {
      List<Integer> resultList = new ArrayList<>();
     ...
     * 迭代思路 (巧妙法)：
     * 后序是 "左 -> 右 -> 根"。
     * 我们可以实现一个 "根 -> 右 -> 左" 的遍历，然后将结果反转，就得到了 "左 -> 右 -> 根"。
     * "根 -> 右 -> 左" 的实现非常类似前序 ("根 -> 左 -> 右")，只需要把压栈顺序反过来 (先压左，再压右) 即可。
     * * 我们使用 LinkedList 的 addFirst() 方法，可以在添加时就实现反转效果。
     */
    List<Integer> postorderTraversal(TreeNode root) {
