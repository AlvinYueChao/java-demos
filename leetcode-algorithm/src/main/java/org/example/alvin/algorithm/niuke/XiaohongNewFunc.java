 ...
 * 59的二级制为（111011)，后缀最大连续1的长度为2，对应的二进制串为（11），即3，因此f(59) = 3，
 * 同样的有f(5)= 1, f(6)= 0;
 * 现在小红想要计算从1到n这n个数中，所有f(i)的和
 * 输入描述：
 * 每个测试文件均包含多组测试数据，第一行输入一个整数T代表数据组数，每组测试数据描述如下：输入一个整数n代表计算范围的上限
 * 输出描述：
 * 对于每组测试数据，新起一行，输出一个整数，表示所有f(i)的和
 * 示例：
 * 输入：
 * 5
 * 1
